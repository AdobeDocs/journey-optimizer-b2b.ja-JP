...
| 機能 | フォーム | マーケターは、再利用可能なフォームコンポーネントを作成して公開し、Journey Optimizer B2B edition _（以前はBeta プログラム機能）_&#x200B;で作成および公開されたランディングページからのデータ送信を有効にできるようになりました。 |
| 機能 | アカウントリストのメンバーフィルター | アカウントリストのメンバーシップをアカウントジャーニーのオーケストレーションポイントとして使用します。 マーケターは、アカウントブロックリスト、優先順位リスト、その他のオーディエンスのサブセットを作成し、メンバーシップにもとづいてジャーニーで独自の体験を提供できます。 |
...
| 機能 | 次善のパスジャーニーノード | 個人ジャーニーの分割パス決定を最適化し、ジャーニーをアクティブ化する前や実行時に、パスをまたいでオーディエンスをルーティングする方法を調整できます。 [詳細情報](../journeys/next-best-path-node.md) |
...
| 機能強化 | メールエンゲージメントパフォーマンスレポート | メールエンゲージメントパフォーマンスレポートでは、マーケターはAdobe Journey Optimizer B2B editionのあらゆるジャーニーをまたいで、メール活動の統一されたビューを確認できます。 |
| 機能強化 | ジャーニー概要ダッシュボード – 人物ジャーニー | 個人ジャーニー（Beta）に、_[!UICONTROL 概要]_ タブが追加され、ジャーニー完了率の分布、エンゲージメントタイプ別のジャーニー、ドリルスルーおよび書き出しアクションなど、アカウントジャーニーと同じ分析が使用できるようになりました。 <a href="../dashboards/journeys-dashboard.md">詳細情報</a> |
