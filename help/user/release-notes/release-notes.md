...
**デプロイメント日**：2026年2月20日（PT）

| タイプ | 項目 | 説明 |
| ---- | ---- | ----------- |
| 機能 | XDM フィールド/リレーショナルスキーマ – 人物カスタムオブジェクトのサポート | （Beta）管理者は、アカウントとの単一レベルの一対一の関係を使用して、人物に関連付けられたカスタムオブジェクトを選択できるようになりました。 この機能を使用すると、マーケティング組織は、実際のビジネスデータをより詳細に把握し、人物レベルやアカウントレベルを超えて、エンティティのターゲット、パーソナライズ、レポートを行うことができます。 [詳細情報](../admin/xdm-field-management.md#relational-schemas) |
| 機能 | ジャーニーの再入場 | ジャーニーワークフローを通じてアカウント/人物を複数回送信できるようになりました。 再入力は、認定条件の再評価や再利用可能な育成ワークフローなど、複数のシナリオに対応します。 [詳細情報](../journeys/journey-re-entry.md) |
| 機能 | メールデザイン - Fireflyとカスタム生成 AI モデルのサポート | 標準およびカスタムのFirefly モデルと、承認済みのサードパーティ画像モデル（NanoBanana など）の統合を有効にできるようになりました。 マーケターは、一般的なニーズに対応する標準Firefly、ブランドに合わせたカスタム Firefly、専門的または実験的なシナリオに対応する承認済みのサードパーティモデルなど、各ユースケースに最適なモデルを選択できます。 |
| 機能強化 | メールデザイン – コンテンツ品質の検証 | ブランドの整合性に加えて、全体的なコンテンツ品質を評価することで、（ブランドガイドラインに依存せずに）読みやすさ、凝集性、有効性に関する潜在的な問題を明らかにすることができます。 これらの自動チェックは、不明確なメッセージ、一貫性のないトーン、構造上のギャップを特定するのに役立ちます。  |
| 機能強化 | アカウントおよび人物のジャーニー – 人物カスタムオブジェクトのサポート | （Beta）アカウントまたはユーザージャーニー内のユーザーをフィルタリングするために、アカウントにリンクされたリレーショナルデータを活用します。 [詳細情報](../journeys/split-merge-paths-nodes.md#custom-data-filtering) |
| 機能強化 | （Beta）コンテンツのパーソナライゼーション – 人物カスタムオブジェクトのサポート | カスタムオブジェクトを使用してコンテンツのパーソナライゼーションを定義する場合、モデルベースのクラスのカスタムオブジェクト（リレーショナルスキーマ）の変数にアクセスできます。 [詳細情報](../content/personalization.md#custom-datasets) |
| 機能強化 | 宛先へのアクティブ化 – 再利用可能なオーディエンス | 同じジャーニー内の _宛先に対するアクティブ化_ ジャーニーアクションで仮想オーディエンスを再利用し、仮想オーディエンスからアカウントを削除できるようになりました。 |
...
| 機能強化 | 購買グループのメンバーの特殊フィルター – 削除された制約 | _[!UICONTROL 購入グループのメンバー]_ 特別フィルターに _削除済み_ 制約が含まれるようになりました。 この制約をフィルターに追加すると、削除された購買グループ メンバーを含めたり、除外したりできます。 また、Marketo Engageのスマートリストでもサポートされており、この新しい制約を _[!UICONTROL 購入グループのメンバー]_ フィルターで使用できます。 |
