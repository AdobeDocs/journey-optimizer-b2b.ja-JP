...
>[!NOTE]
>
>これらのリリースの変更は、2026年5月22日（PT）にデプロイメントが開始され、各機能の段階的なロールアウトと機能強化が行われます。 機能および機能強化のリリース日は変更される場合があります。
...
| タイプ | 項目 | 説明 |
| ---- | ---- | ----------- |
| 機能 | WhatsApp チャネル | マーケターは、[!DNL Meta Cloud API]を通じて、アカウントジャーニーから直接WhatsApp メッセージを送信できるようになりました。 この機能により、WhatsApp メッセージのシームレスな統合が可能になり、ジャーニーコンテンツチャネルをサポートできます。 [詳細情報](../admin/configure-channels-whatsapp.md) |
...
| 機能 | ジャーニーのカスタム外部アクション | 開発者は、APIを使用して、ファーストパーティシステムとの統合を構築できるようになりました。 これらのカスタム統合を使用すると、マーケターは&#x200B;_外部アクション_&#x200B;および&#x200B;_外部スプリットパス_ ノードを追加して、アカウントジャーニーの実行中に外部サービスにアウトバウンドリクエストを行うことができます。 [詳細情報](../admin/configure-external-actions.md) |
...
| 機能強化 | メール送信 – 送信時間の最適化 | 対面ジャーニーの&#x200B;_メール送信_ アクションノードの場合、_送信時間の最適化_ オプションを使用して、各プロファイルがエンゲージする可能性が最も高いタイミングを予測し、メール配信のタイミングをパーソナライズできます。 [詳細情報](../content/email-send-time-optimization.md) |
...
| 機能強化 | 人物カスタムオブジェクト – 購買グループの役割テンプレート | 管理者がビジネスパーソンのプロファイルに関連するカスタムオブジェクトを設定すると、マーケターはこれらのカスタムオブジェクトを使用して購買グループの役割を定義できるようになりました。 [詳細情報](../buying-groups/buying-groups-role-templates.md#add-the-template-roles) |
...
| タイプ | 項目 | 説明 |
| ---- | ---- | ----------- |
| 機能 | XDM フィールド/リレーショナルスキーマ – 個人カスタムオブジェクトのサポート | （Beta）管理者は、アカウントとの1対1の関係を使用して、個人に関連するカスタムオブジェクトを選択できるようになりました。 この機能により、マーケティング部門は、個人やアカウントレベルを超えたエンティティをターゲット化、パーソナライズ、レポート作成するために、実際のビジネスデータをより詳細に把握できるようになります。 [詳細情報](../admin/xdm-field-management.md#relational-schemas) |
| 機能 | ジャーニーの再入場 | ジャーニーワークフローを通じて、アカウント/人物を複数回送信できるようになりました。 再入力は、クオリフィケーション基準の再評価や再利用可能なナーチャリングワークフローなど、複数のシナリオに対応します。 [詳細情報](../journeys/journey-re-entry.md) |
| 機能強化 | アカウントと人物のジャーニー – 人物カスタムオブジェクトのサポート | （Beta） アカウントまたはユーザージャーニー内の人物をフィルタリングするために、アカウントにリンクされている関連データを活用します。 [詳細情報](../journeys/split-merge-paths-nodes.md#custom-data-filtering) |
| 機能強化 | （Beta） コンテンツのパーソナライゼーション – ユーザーのカスタムオブジェクトのサポート | マーケターがカスタムオブジェクトを使用してコンテンツパーソナライゼーションを定義すると、モデルベースのクラスカスタムオブジェクト（リレーショナルスキーマ）の変数にアクセスできます。 [詳細情報](../content/personalization.md#custom-datasets) |
...
| 機能 | 購買グループのカスタムロール設定 | 購買グループのカスタムロールを柔軟に設定できるようになりました。これにより、ユースケースに固有のロールを定義できます。 [詳細情報](../buying-groups/default-custom-roles.md) |
...
| タイプ | 項目 | 説明 |
| ---- | ---- | ----------- |
| 機能 | GenStudio for Performance Marketing との統合 | （限定提供）GenStudio for Performance Marketing のメールエクスペリエンスを Journey Optimizer B2B Edition と統合して、マーケティング効率を高め、ブランドの一貫性を維持できるようになりました。 この統合を利用すると、GenStudio の AI を活用したコンテンツ制作と、Journey Optimizer B2B Edition の高度なオーケストレーション機能を組み合わせることができます。 [詳細情報](../content/genstudio-email-workflow.md) |
...
| 機能強化 | メールのハンドルバートークン形式 | メールコンテンツのパーソナライゼーショントークンで、Handlebar スクリプティングと完全に互換性のある更新済み形式が使用されるようになりました。 この形式では、_キャメルケース_&#x200B;またはアンダースコアを使用し、スペースを排除します。 [詳細情報](../content/email-authoring.md#personalize-content) |
...
| 機能 | 有料メディアの宛先 | アカウントジャーニーから有料メディアキャンペーンの対象となる既知の人物を選定し、LinkedIn などの広告プラットフォームでさらにエンゲージメントを図ることができます。 分割パスノードを使用して、特定の行動に基づいてアカウントオーディエンスをセグメント化し、追加のエンゲージメントが必要なアカウントを識別します。 次に、Real-Time CDP を通じて、これらのアカウントの人物を外部顧客オーディエンスに追加し、そのオーディエンスをサポートされている有料メディアの宛先に送信します。 <a href="../journeys/action-nodes.md#journey-optimizer-b2b-actions">詳細情報</a> |
| 機能 | インテリジェントダッシュボード | アカウントジャーニーにおける購買グループの進行状況を表示し、よりインテリジェントな分析と正確なアカウントの優先順位付けのための AI 生成インサイトを確認できます。 <a href="../dashboards/intelligent-dashboard.md">詳細情報</a> |
...
| 機能 | ジャーニー概要ダッシュボード | アカウントジャーニーの場合、「**[!UICONTROL 概要]**」タブには、アクティブなジャーニーの包括的なスナップショットが表示され、完了とエンゲージメント活動を分類および定量化する円と棒グラフが表示されます。 <a href="../dashboards/journeys-dashboard.md">詳細情報</a> |
...
| 機能 | コンテンツガバナンスとコンポーネントのロック | 承認済みのコンテンツデザインに準拠するには、コンテンツガバナンス機能を使用して、メールテンプレートのコンテンツコンポーネントをロックします。 メールテンプレートでコンテンツガバナンスを有効にすると、マーケターは許可された要素のみを変更して、コンテンツ戦略に合わせた状態を維持できます。 <a href="../content/template-content-governance.md">詳細情報</a> |
...
| 機能強化 | 新しい標準メールテンプレート | サンプルテンプレートライブラリに、B2B マーケター向けに設計された追加のメールテンプレートが含まれるようになりました。 これらのサンプルテンプレートを出発点として使用し、独自のブランディングとメッセージングを追加します。 <a href="../content/email-templates.md#select-a-design-template">詳細情報</a> |
| 機能強化 | ユーザー属性としてのカスタムフィールド | Experience Platform のアカウントオーディエンススキーマでカスタムの人物フィールドが定義されている場合は、これらのフィールドを条件内で人物属性として使用することもできます。 これらのカスタム属性は次で使用します。 <li>役割テンプレート<a href="../buying-groups/buying-groups-role-templates.md#add-the-template-roles">詳細情報</a></li><li>人物ジャーニーノード別にパスを分割<a href="../journeys/split-merge-paths-nodes.md#add-a-split-path-by-people-node">詳細情報</a></li> |
| 機能強化 | メールチャネル設定 | メール設定が Adobe Journey Optimizer B2B Edition のインターフェイスに表示されるようになりました。 現在の設定をすばやく確認でき、管理者は「_[!UICONTROL 設定を編集]_」をクリックして Marketo Engage の設定に直接移動し、組織の要件に応じて設定を更新できます。 <a href="../admin/configure-channels-emails.md">詳細情報</a> |
...
| 機能強化 | ビジュアルコンテンツフラグメントのステータスライフサイクル | ビジュアルフラグメントは、ステータスライフサイクルを使用して管理されるようになりました。 フラグメントのステータスによって、メールまたはメールテンプレートでの使用可否と、そのフラグメントにどのような変更を加えられるかが決まります。 <p>この強化されたワークフローにより、プロモーションや通信のカレンダーに従って再利用されたコンテンツを簡単に管理できます。 <a href="../content/fragments.md#fragment-status-and-lifecycle">詳細情報</a> |
