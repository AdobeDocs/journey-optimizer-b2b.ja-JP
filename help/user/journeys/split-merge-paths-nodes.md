...
| パス条件 | 説明 |
| --------------- | ----------- |
| [!UICONTROL &#x200B; アカウント属性 &#x200B;] | アカウントプロファイルからの属性。次のものが含まれます。 <li>年間売上高 <li>市区町村 <li>国 <li>従業員数 <li>業界 <li>名前 <li>SIC コード <li>State |
| [!UICONTROL &#x200B; アカウント属性 &#x200B;]/Has `<custom object>` | [!BADGE Beta]{type=Informative tooltip="Beta機能"} アカウントにリレーショナルスキーマレコードが含まれているか、含まれていません。 また、[XDM リレーショナルスキーマ &#x200B;](../admin/xdm-field-management.md#relational-schemas) で設定しているように、選択したカスタムオブジェクト条件のいずれかに照らして評価することもできます。 （[&#x200B; カスタムデータフィルタリング &#x200B;](#custom-data-filtering) を参照）。 |
...
| [!UICONTROL &#x200B; 人物の属性 &#x200B;] | ユーザープロファイルからの属性。以下が含まれます。 <li>市区町村 <li>国 <li>生年月日 <li>メールアドレス <li>メール無効 <li>メール中断済み <li>名 <li>推測される都道府県 / 地域<li>役職 <li>姓 <li>携帯電話番号 <li>人物エンゲージメントスコア <li>電話番号 <li>郵便番号 <li>ステート <li>配信停止完了 <li>登録解除の理由 <li>`<custom object>` がある（[&#x200B; カスタムデータフィルタリング &#x200B;](#custom-data-filtering) を参照）。 |
| [!UICONTROL Person Attributes] > `<custom object>` | [!BADGE Beta]{type=Informative tooltip="Beta機能"} その人物はリレーショナルスキーマレコードを持っていないか、持っていません。 また、[XDM リレーショナルスキーマ &#x200B;](../admin/xdm-field-management.md#relational-schemas) で設定しているように、選択したカスタムオブジェクト条件のいずれかに照らして評価することもできます。 （[&#x200B; カスタムデータフィルタリング &#x200B;](#custom-data-filtering)）を参照。 |
...
_[!UICONTROL 特別なフィルター]_ セクションには、購入グループまたは [!DNL Marketo Engage] ーザーリスト内の人物のメンバーシップを評価するために使用できる複数のフィルターがあります。

例えば、購入グループのメンバーで特定の役割が割り当てられている人物のパスを作成する場合は、_[!UICONTROL 特別なフィルター]_/_[!UICONTROL 購入グループのメンバー]_ フィルターを追加します。 フィルターの場合、メンバーシップを _true_ に設定し、1 つ以上の購入グループに関連付けられている _[!UICONTROL ソリューションの関心]_ を選択して、一致させる _[!UICONTROL 役割]_ を設定します。

![&#x200B; 購買グループ メンバーシップの条件でパスを人物別に分割 &#x200B;](./assets/node-split-people-condition-buying-group-membership.png){width="700" zoomable="yes"}

追加の購買グループ・メンバーシップ制約を含めることもできます。

* _[!UICONTROL 購買グループステージ]_
* _[!UICONTROL 購入グループのステータス]_
...
>[!TIP]
>
>購買グループから削除されたメンバーを除外するには、「_[!UICONTROL 削除]_」制約を `false` に設定します。 この制約を `true` に設定することで、削除されたメンバーを明示的に含めることもできます。

>[!BEGINSHADEBOX  「Marketo Engage リストとプログラムメンバーシップ」 ]
...
分割条件でリスト メンバシップを使用するには、[**[!UICONTROL 特殊フィルタ]**] を展開し、[**[!UICONTROL リストのメンバ]**] または [**[!UICONTROL プログラムのメンバ]**] 条件をフィルタ スペースにドラッグします。 フィルター定義を完了して、1 つ以上の [!DNL Marketo Engage] リストのメンバーシップを評価します。
...
>**機能の廃止**</br></br>
>
>Journey Optimizer B2B editionの [&#x200B; シンプルなアーキテクチャ &#x200B;](../simplified-architecture.md) では、Marketo Engage インスタンスのリストまたはプログラムメンバーシップに基づくフィルタリングはサポートされていません。
...
リレーショナルスキーマ（モデルベースのクラス）を使用して、パスをアカウント別またはユーザー別に分割できます。 カスタムオブジェクトは _リレーショナルスキーマ_ 内で定義され、製品管理者は [&#x200B; で &#x200B;](../admin/xdm-field-management.md#relational-schemas) リレーショナルスキーマフィールドを設定 [!DNL Journey Optimizer B2B Edition] することができます。 選択したスキーマフィールドは条件エディターで使用でき、_アカウントによるパスの分割_ および _人物によるパスの分割_ ノードで使用できます。

**[!UICONTROL アカウントでパスを分割]** 条件の場合は、検索フィールドを使用して、_[!UICONTROL アカウント属性]_ の下にあるカスタムオブジェクト名でリストをフィルタリングします。 条件を追加し、値を `true` または `false` に設定します。

![&#x200B; リレーショナルスキーマカスタムオブジェクトに対する人物属性の条件の例 &#x200B;](./assets/node-split-paths-account-relational-schema.png){width="600" zoomable="yes"}

**[!UICONTROL ユーザーでパスを分割]** 条件の場合は、検索フィールドを使用して、_[!UICONTROL ユーザー属性]_ の下にあるカスタムオブジェクト名でリストをフィルタリングします。

![&#x200B; リレーショナルスキーマカスタムオブジェクトに対する人物属性の条件の例 &#x200B;](./assets/node-split-paths-people-relational-schema.png){width="600" zoomable="yes"}
...
_結合パス_ ノードを追加して、ジャーニーの様々な _アカウント別の分割パス_ を組み合わせます。
