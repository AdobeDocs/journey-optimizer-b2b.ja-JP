...
| 入力タイプ | イベント | 制約 |
| ---------- | ----- | ----------- |
| Journey Optimizer B2B | [!UICONTROL 購買グループに割り当て] | ソリューションの関心<br/><br/>追加の制約（オプション）: <li>ロール</li><li>アクティビティの日付</li><br/> タイムアウト （オプション） |
...
1. ジャーニーマップのイベント ]**ノードの**[!UICONTROL  リッスンを選択します。
...
1. 「**[!UICONTROL イベントを編集]**」をクリックし、一致する1つ以上のweb ページと、イベントの追加の制約を定義します。
   ...
   * 必要に応じて、「**[!UICONTROL フィルター]**」タブを選択して、イベント ](#add-a-filter-to-the-people-event)のフィルターを[追加します。
