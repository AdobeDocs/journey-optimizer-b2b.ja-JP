...
description: 再利用可能なテーマを使用してメールコンテンツの作成を合理化し、デザインの一貫性と効率を確保する方法について説明します。
...
# メールコンテンツにブランドテーマを使用 {#email-brand-themes}

>[!CONTEXTUALHELP]
>id="ajo-b2b_email_brand_theme"
>title="メールまたはメールテンプレートへのブランドテーマの適用"
>abstract="メールまたはメールテンプレートのテーマを選択して、ブランドやデザインに合った特定のスタイルを適用します。"
...
* 「**[!UICONTROL バリアントを追加]**」をクリックして、_明_ モードや _暗_ モードなど、複数のカラーバリアントを作成します。各バリアントには、独自のカラーパレットとニュアンスのコントロールがあります。 最大 6 つのバリアントを使用できます。
  ...
  | タイプ | 設定 | 説明 |
  | ---- | -------- | ----------- |
  | [!UICONTROL &#x200B; 一般 &#x200B;] | ![ バリアントの一般的なカラー設定 ](./assets/email-theme-colors-settings-variant-general.png){width="300"} | これらの設定を使用すると、ボディ、構造物、コンテナ、背景、リンク、グリッド、および境界の色を設定できます。 |
...
| [!UICONTROL &#x200B; 整合性 &#x200B;] | ![ ディバイダーの位置揃えの設定 ](./assets/email-theme-divider-settings-alignment.png){width="300"} | 含まれるブロック内の水平方向の配置を _左_、_中央_、または _右_ から選択します。 |
...
新しいメールまたはメールテンプレートを作成する際には、コンテンツオーサリングプロセスを合理化し、定義済みの標準にデザインが準拠していることを確認するブランドテーマを使用できます。 新しいフラグメントの場合、フラグメントを保存する前にテーマを適用することもできます。 フラグメントは、その時点から _テーマモード_ のままであり、同じく _テーマモード_ のメールおよびメールテンプレートに追加するための互換性があります。
