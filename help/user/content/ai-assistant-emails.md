...
メールプリヘッダーとは、電子メールが受信トレイに表示されるときに、件名の後に続く短い要約テキストのことです。 メールのオプション要素ですが、エンゲージメントを向上させる効果的な機会です。 次の手順では、AI アシスタントを使用してメール用に最適化されたプリヘッダーを生成するためのタスクシーケンスについて説明します。
...
既存のメールテンプレートを改良して、AI アシスタントをメール全体の生成に使用するには、次の手順に従います。
...
* **[!UICONTROL 要約]** – 長い情報で読者が過負荷になる可能性があります。 AI アシスタントを使用して、重要なポイントを明確かつ簡潔な概要に要約し、注意を引いてさらに読むよう促します。
