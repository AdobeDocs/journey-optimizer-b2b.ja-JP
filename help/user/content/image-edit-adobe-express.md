...
[!DNL Adobe Journey Optimizer B2B Edition] はAdobe Expressとネイティブに統合されており、一連の [!DNL Adobe Express] 画像編集ツールにアクセスできます。 これらのツールを使用して、[!DNL Journey Optimizer B2B Edition] アセットリポジトリに保存されている画像を変更できます。 この統合には、次のような主なメリットがあります。
...
[!DNL Adobe Express] の関数は、PNG およびJPEGの画像ファイル形式をサポートします。
...
1. Adobe Express クイックアクションエディターで、画像編集機能のいずれかを選択して画像を変更します。
   ...
   * [**[!UICONTROL 背景の削除]**](#remove-background)
   * [**[!UICONTROL 画像を切り抜き]**](#crop-image)
