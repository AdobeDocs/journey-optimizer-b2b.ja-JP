...
1. Marketo Engage ランディングページを使用する場合は、トラッキングオプションを設定します。
...
1. コミュニケーションに使用する **[!UICONTROL 送信者番号]**&#x200B;を入力します。
   ...
   ![SMS メッセージのプロパティ ](./assets/sms-properties.png){width="700" zoomable="yes"}
...
このオプションを指定すると、SMS 受信者はオプトインおよびオプトアウトのキーワードを使用して返信できます。 すべての標準のオプトインおよびオプトアウトキーワード、および SMS サービスプロバイダーで設定されるカスタムキーワードがサポートされ、適用されます。 購読を解除すると、プロファイルは、今後のマーケティングメッセージのオーディエンスから自動的に削除されます。
