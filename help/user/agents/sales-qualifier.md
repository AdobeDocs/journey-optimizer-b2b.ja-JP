...
* _この電子メールの目的は、会話を開始して信頼性を高めることです。_ コンサルタント的で共感的なトーンの120語に満ちたメールを作成しましょう。 過度に使い慣れたセールスアプローチを避け、「うまくいくことを願う」、「チェックインするだけ」、「お願いします」というフレーズを使用しないでください。
...
メールのタッチポイントを拡大し、プロンプトを読む。 この手順では、トーン、長さ、焦点、call to actionなど、各見込み客のメールを作成する際のAIをガイドします。
...
* **見込み客が生成される前に、タッチポイントプロンプトを最終決定します。****一括生成後、通常、変更は一度に1人の見込み客に対して行われます。
* **推論を品質チェックとして使用します。** 誤ったシグナルが強調されたり、見落とされたりした場合は、電子メールを編集するか、タッチポイントプロンプトを再訪問して、ケイデンスを再生成します。
* **編集ツールを変更に合わせます。****文言とトーンの直接編集、**[!UICONTROL AIを利用して生成]**、再構築またはリフレーム。
* **レビューしたものだけを承認してください。****顧客接点を拡張し、コンテンツを読み取り、必要に応じて登録前に調整してください。
