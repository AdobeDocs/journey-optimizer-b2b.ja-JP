...
![AIを活用したセールス見込み顧客とアカウントの調査用のAccount Qualification Agent ダッシュボード ](./assets/acc-qualification-agent.png){width="800" zoomable="yes"}
...
* **下書きメール**
  ...
  見込み顧客やアカウントのインサイトから得たリサーチを統合して、BDRの目標にもとづいて、パーソナライズされた関連性の高い単一メールコンテンツを生成することで、メールのドラフトを生成できます。
...
* _収益の呼び出しやレポートなどのコンテキストからフォローアップ メールの描画を作成します。 最大120単語。 件名：魅力的で、重要なテーマを取り入れている はじめに：コンテキストソースから直接の引用でフックします。 本文：課題と価値提案につながる CTA：さらに詳しく調べるために短い呼び出しを提案します。_

* _この電子メールの目的は、会話を開始して信頼性を高めることです。 コンサルタント的で共感的なトーンの120語に満ちたメールを作成しましょう。 過度に使い慣れたセールスアプローチを避け、「うまくいくことを願う」、「チェックインするだけ」、「お願いします」というフレーズは使用しないでください。_
...
左側のナビゲーションで「**[!UICONTROL 見込み客]**」を選択して、アクセス可能なすべてのリードのリストを表示します。 リードのステータスや最後のアクティビティなど、素早く確認できます。
...
ワークフローの作成は5段階のウィザードです：**目標**、**ターゲット**、**顧客接点の生成**、**設定**、および&#x200B;**見込み客の追加**。 各ステップは最後のステップの上に構築され、後の決定ごとに「最初のシェイプ」を定義することを目標としています。
...
ターゲティングフィルターで適格な見込み客を定義。 後で見込み客を追加すると、これらのフィルターに一致する人のみが選択リストに表示されます。
...
電子メールのタッチポイントを展開し、そのプロンプトを読みます。このプロンプトは、AIが見込み客ごとに実際の電子メールを作成する際に従う手順です（トーン、長さ、フォーカス、call to action）。
...
生成された各電子メールについて、**[!UICONTROL 推論]**&#x200B;は、AIがメッセージをどのように作成したかを説明しています。これには、コンテンツとcall to actionを形作ったシグナル、属性、ソースが含まれます。 承認する前に、この情報を確認してパーソナライゼーションを検証します。
...
ワークフロー&#x200B;**送信ウィンドウ** （設定された&#x200B;**タイムゾーン**&#x200B;内）で、登録に関連する各タッチポイントのスケジュールされた日に送信するための承認済み電子メールキュー。 承認していない見込み客は、行動するまで&#x200B;**[!UICONTROL レビューの準備状態]**&#x200B;のままです。 承認後、定義したケイデンスに従ってワークフローが実行されます。
...
セールス修飾子の&#x200B;_タスク_&#x200B;領域は、ビジネス開発担当者（BDR）に、アウトバウンドワークフローアクションを管理および処理するための専用のスペースを提供します。 タスクは、アウトバウンドワークフローエンジンによって自動的に生成され、電話、LinkedInMails、メールレビューなど、BDRが見込み客ごとに取る必要がある特定のアクションを表します。

タスク管理エクスペリエンスは、ToDo リストだけでなく、**処理キュー**&#x200B;として設計されています。 ページを離れることなく、タスクを開いてアクションを実行し、タスクの完了をマークして、次のタスクに移動できます。

左側のナビゲーションバーで「**[!UICONTROL タスク]**」を選択して、完全なタスクページを開きます。 タスクを一つずつ処理するための主要なワークスペースです。
...
**メールレビュー** — ワークフローに登録されている見込客に対するパーソナライズされたメールの生成がシステムによって完了すると作成されます。 見込み客のアウトバウンドが始まる前に、メールを確認し、承認します。 見込み客ごとにメールレビュータスクが割り当てられます。ワークフローに10人の見込み客を登録すると、生成完了時に最大10件のメールレビュータスクが表示されます。
...
* **[!UICONTROL 完了をマーク]** – 主なアクション。 これは、タスクを実行した後に使用します。呼び出しを行ったり、InMailを送信したり、メールを確認して承認したりします。 完了すると、タスクは&#x200B;**完了**&#x200B;として記録され、キューは自動的に進みます。

* **[!UICONTROL タッチポイントをスキップ]** – 作業パネルのオーバーフローメニューから使用できます。 この特定のステップを完了できなくても、見込み客がまだ有効なターゲットであり、ワークフローに残っている必要がある場合に使用します。

  タッチポイントをスキップすると、次のようになります。
   * 見込み客はシーケンスの次のステップに進みます。 今後のタスクはスケジュール通りに生成されます。
   * 理由を選択する必要があります：*不正な連絡先情報*、*不正なタイミング*、*関連しないコンテンツ*、または&#x200B;*その他* （フリーテキストフィールドあり）。
   * タスクのステータスは&#x200B;**スキップ**&#x200B;に設定され、理由とタイムスタンプが記録されます。
   * これがワークフローの最後のステップだった場合、見込み客のワークフロー実行は終了します。 タスクはスキップ済みとして記録されます（削除されません）。

* **[!UICONTROL ワークフローから削除]** – 作業パネルのオーバーフローメニューから使用できます。 見込み客がこのワークフローに入るべきではない場合に使用します。

  ワークフローから見込み客を削除する場合：
   * このワークフロー内のその見込み客に対する保留中および今後のタスクはすべてキャンセルされます。
   * 見込み客の登録ステータスが&#x200B;**BDR**&#x200B;によって削除されました。
   * 理由を選択する必要があります：*Left company*、*Duplicate*、*Wrong fit*、*既に変換済み*、または&#x200B;*その他* （テキストフィールドあり）。
   * 確認ダイアログが表示されます：*&quot;これにより、[ ワークフロー名]の[見込み客]の残りのすべてのタッチポイントがキャンセルされます。 続行しますか？&quot;*
   * タスクの状態は&#x200B;**削除済み**&#x200B;に設定されています。 キャンセルされたすべての兄弟タスクも&#x200B;**削除済み**&#x200B;とマークされます。

>[!NOTE]
>
>理由データのフィードをスキップして分析に取り込む – チャネル別のスキップ率、ワークフロー別の取り込み率、および上位の理由。 これにより、ワークフローの品質を向上させ、長期的なパフォーマンス分析に情報を提供できます。
...
* **期限切れ** – 期限が過ぎて、まだ開いているタスク。 まずはこれらに対処してください。

* **今後** – 前のワークフローステップが既に完了した期日を持つタスク。 これらは早い段階で確認できるため、適切なタイミングで計画を立てたり、より早く行動したりすることができます（たとえば、見込み客との電話を既に行っている場合など）。 予定期日が表示されるので、意図したタイミングがわかります。
...
次のステップの期日がまだ後である場合でも、見込み客がワークフローステップを完了した瞬間に今後のタスクが作成されます。 これにより、パイプラインを早期に可視化でき、事前に計画を立てたり、機会が発生した際に早期に行動したりすることができます。
...
>[!IMPORTANT]
>
>Sales Qualifierの統合にアクセスするには、`Sales Qualifier Admins` ユーザーグループのメンバーシップが必要です。
...
Salesforceでは、組織のセキュリティ基準に従って、OAuthを有効にし、IDとデータへのAPI アクセスを許可するスコープを含む外部クライアントアプリ ](https://help.salesforce.com/s/articleView?id=xcloud.create_a_local_external_client_app.htm) （または&#x200B;_接続アプリ_）を[作成します。 統合ユーザー（クライアント資格情報スタイル設定を使用する場合など）は、リード、アカウント、取引先責任者、タスク、イベント、商談、関連する商談オブジェクトなどのオブジェクトに対する読み取りアクセス権を持っている必要があります。 管理タスクでは、作成後にコンシューマーキーとシークレットを表示するために、**[!UICONTROL 接続されたアプリを管理]**&#x200B;するユーザー（その他の権限を含む）が必要になることがよくあります。

>[!PREREQUISITES]
>
>外部クライアントアプリを作成するには、システム管理者で、（プロファイルまたは権限セットから）次の機能が有効になっていることを確認する必要があります。
>
>* アプリケーションをカスタマイズ
>* 設定と設定の表示
>* すべてのデータを変更
...
* 予約ページに、フレンドリーで一時的に利用不可の状態が表示されます。
* 再接続すると、設定が保持されます。
