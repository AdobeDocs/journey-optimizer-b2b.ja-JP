...
[Audience Agent B2B](../agents/audience-agent-b2b.md)は、ファーストパーティのインテント検出とペルソナマッピングから購買グループの役割テンプレートを生成できます。 ガイド付きのフローでは、製品にリンクされたペルソナを特定し、AIが推奨する役割とペルソナのマッピングを確認し、公開する前に自然言語でテンプレートを絞り込むことができます。

**次の操作を試すプロンプト：**

* `<product>`の購買グループテンプレートを作成
* `<persona>`にマッピングされた`<role>`を追加
* `<role>` / `<persona>`を削除

![購買グループのロールテンプレートを作成するAudience Agent B2B](./assets/buying-group-roles-agent-create.png){width="800" zoomable="yes"}
...
### テンプレートの役割の追加 {#add-the-template-roles}

テンプレートを作成すると、ワークスペースが開き、役割を追加するように求められます。 デフォルトでは、最初の役割カードが表示されます。
...
1. 最初の役割カードに対して、役割のプロパティを定義します。
   ...
   * エンゲージメントスコアの計算に使用する役割の&#x200B;**[!UICONTROL 重み付け]**&#x200B;を設定します。
     ...
     例えば、重要、重要、通常の役割を持つロールテンプレートは、240人中100、80、60人に変換されます。
...
1. 「**[!UICONTROL 条件を追加]**」をクリックし、役割の条件付きルールを定義します。
   ...
   * _[!UICONTROL 条件]_ ダイアログで、**[!UICONTROL 人物属性]**&#x200B;のリストを展開し、役割に一致させるために使用する属性を見つけます。 右にドラッグして、フィルタースペースにドロップします。
     ...
     >[!NOTE]
     >
     >Experience Platform ビジネス人物スキーマでカスタム人物フィールドを定義している場合は、これらのフィールドを条件の人物の属性として使用できます。
...
1. ダイアログで、**[!UICONTROL 削除]**&#x200B;をクリックして確認します。
   ...
   確認すると、役割テンプレートが&#x200B;**[!UICONTROL 役割テンプレート]** インベントリ リストから削除されます。
