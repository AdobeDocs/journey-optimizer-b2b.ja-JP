...
description: 条件付き自動割り当てを含むロールテンプレートを作成し、Journey Optimizer B2B editionで購買グループの意思決定者と関係者を特定します。
...
| タイプ | 条件 |
| ---- | --------- |
| 顧客属性 | <li>メールアドレス <li>メール無効 <li>メール中断済み <li>FAX 番号 <li>名 <li>推測される都道府県 / 地域 <li>役職 <li>姓 <li>ミドルネーム <li>携帯電話番号 <li>人物エンゲージメントスコア <li>電話番号 <li>郵便番号 <li>ステート <li>配信停止完了 <li>登録解除の理由 |
| 特殊フィルター | <li>リストのメンバー <li>プログラムのメンバー |
| インテントデータ | <li>カテゴリインテント <li>製品インテント <li>キーワードのインテント <br/>[&#x200B; インテントデータについて学ぶ &#x200B;](../admin/intent-data.md) |
...
>**機能の廃止**</br></br>
>
>Journey Optimizer B2B editionの [&#x200B; シンプルなアーキテクチャ &#x200B;](../simplified-architecture.md) では、のMarketo Engage インスタンスのリストまたはプログラムメンバーシップに基づくフィルタリングはサポートされていません。
...
デフォルトでは、役割の完全性は、役割に割り当てられた 1 つのメンバーとして定義されます。 購入グループの完全性を営業準備状況または成功 <!-- journey decisioning coming later--> の指標として使用する場合は、これらの設定を使用して、スコアを商談をクローズするために必要な役割ごとのメンバー数に合わせることができます。

例えば、ソリューション _X_ の取引を成立させるには、組織全体の複数のマーケティングチームがソリューションを使用するので、複数のマーケティング意思決定者を特定して関与する必要があります。 この場合、少なくとも 2 人のマーケティング意思決定者が必要で、_完了_ 購入グループを計算するしきい値を増やす必要があります。
...
1. ダイアログで、定義された各役割の **[!UICONTROL Members required]** 値を必要に応じて変更します。

   値を入力するか、**&plus;** または **−** をクリックして値を増減させることができます。
