...
| 特殊フィルター | <li>リストのメンバー |
| インテントデータ | カテゴリインテント <li>製品の意図 <li>キーワードインテント <br/>[ インテントデータについて説明します ](../admin/intent-data.md)。 |
...
Marketo Engageとして、_スマートキャンペーン_ は、プログラムのメンバーシップを確認して、リードが重複したメールを受け取らず、同時に複数のメールストリームのメンバーにならないようにします。 Journey Optimizer B2B では、Marketo Engageリストメンバーシップをロールテンプレートの条件として確認することで、購入グループメンバーシップとジャーニーアクティビティの重複を排除できます。

リスト メンバシップをロール条件として使用するには、[**[!UICONTROL 特殊フィルタ]**] を展開し、[**[!UICONTROL リストのメンバ]**] 条件をフィルタ スペースにドラッグします。 次に、フィルター定義を完了して、1 つ以上のMarketo Engageリストのメンバーシップを評価します。

![Marketo Engageリスト メンバーシップのロール テンプレートの条件 ](assets/roles-template-conditions-member-of-list.png){width="700" zoomable="yes"}
...
テンプレートを使用する準備が整ったら、右上の **[!UICONTROL Publish]** をクリックします。
