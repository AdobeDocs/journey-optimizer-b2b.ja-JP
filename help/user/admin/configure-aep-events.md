...
## ガイドラインと制限事項 {#guidelines-and-limitations}
...
### イベントリストのフィルタリング {#filter-the-event-list}
...
「_[!UICONTROL イベント]_」タブのリストに、保存されたイベントが表示されます。

### イベントの編集 {#edit-an-event}

フィールドを変更するには、イベントの詳細を編集します。
...
1. 「**[!UICONTROL フィールドを編集]**」をクリックして、_[!UICONTROL フィールドを選択]_ ダイアログを開き、さらにフィールドを追加します。
   ...
   このイベントを使用するジャーニーが公開された後に以前に選択したフィールドを削除することはできません。

1. 「**[!UICONTROL 選択]**」をクリックして、選択内容を保存します。

### イベントの削除 {#remove-an-event}

エクスペリエンスイベントがジャーニー内の&#x200B;_イベントのリッスン_ ノードで使用されないようにするには、イベントを削除します。 _スケジュール済み_、_ライブ_、または&#x200B;_完了_ ステータスのジャーニーが使用している場合、イベントを削除することはできません。

1. イベント名の横にある&#x200B;_詳細メニュー_ （**...**）アイコンをクリックし、**[!UICONTROL 削除]**&#x200B;を選択します。
...
| メールソースキー | `directMarketing.emailSent.mailingKey.sourceKey` |
...
### Web リンクをクリックしました {#web-link-clicked}
