...
1. Adobe IDの電子メール、ユーザー名、名、または姓を入力して検索します。

   * アクセス権が正しく設定されている場合、検索はレコードを返します。

   * **[!UICONTROL 管理者ロール]**&#x200B;列の値に`System`が表示されている場合、表示されるユーザーはシステム管理者です。
...
>[!TIP]
>
>従来のデプロイメントから移行して既存のユーザーをMarketo Engageに追加する場合、ユーザーグループの作成をスキップし、既存のグループを開いてMarketo Engage製品プロファイルを追加するだけです。
...
1. 各ユーザーを追加します。
   ...
   * ユーザーのメールアドレス、名、姓を入力します。
     ...
     ![Experience Platform – 新しいロールのユーザープロファイルを追加](./assets/admin-console-add-users.png){width="600" zoomable="yes"}
...
権限は、製品プロファイルに割り当てる許可を定義できる単一の権利です。 各権限は、_ジャーニー_&#x200B;または&#x200B;_購買グループ_&#x200B;などの機能にグループ化されます。 これらの機能は、Journey Optimizer B2B Editionの関数またはオブジェクトを表します。
...
| B2B ジャーニー | B2B ジャーニーの権限を設定、管理、表示、公開します。 これらの権限には、アカウントと人物のアクション、イベントリスナー、分割パスが含まれます。 | <li>B2B ジャーニーの管理 |
...
| B2B セールスユーザー | <li>B2B エンゲージメントダッシュボードの表示 <li>CRM 内インサイトへのアクセス |

### 役割の権限の編集

組み込みの役割またはカスタムの役割の場合は、いつでも権限を追加または削除できます。 デフォルトまたはカスタムの役割を変更すると、その役割に割り当てられたすべてのユーザーに影響します。

次の例では、B2B チャネルマネージャーのロールに割り当てられたユーザーに対して、B2B ジャーニーリソースに関連する権限を追加します。 この変更により、その役割のユーザーはアカウントジャーニーも管理できるようになります。
...
1. **[!UICONTROL グループを追加]** をクリックします。
   ...
   ![Experience Platform - グループをロールに追加](./assets/aep-permissions-role-add-groups.png){width="700" zoomable="yes"}
...
1. B2B 製品の権限を追加します。
   ...
   [B2B製品権限](#b2b-product-permissions)のリストを参照して、役割に必要な製品機能を決定してください。
