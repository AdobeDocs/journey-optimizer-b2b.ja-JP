...
1. SPF を設定するには、DNS エントリに次の行を追加します。
   ...
   DNS エントリにSPF レコードがある場合は、次のレコードを追加します。
...
また、`FROM:` ドメインにDMARC固有のDNS TXT レコードを設定します。 必要に応じて、DMARC レポートを組織内のどこでレポートを監視するかを指定するメールアドレスを定義します。
...
これらのレポートの主な用途は、スプーフィングを試みているメールの概要を受け取ることです。 非常に専門的なレポートであり、サードパーティのツールを通じて分析するのが最適です。
